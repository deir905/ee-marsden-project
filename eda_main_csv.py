...
def analyseEDA(output_dir):
    ...
    for idx, (start_index, end_index) in enumerate(indices):
        ...
        data = nk.eda_phasic(eda_data[start_index:end_index], sampling_rate=sample_rate)
        ...
        with open(os.path.join(output_dir, 'eda_analysis.csv'), 'a', newline='') as f:
            ...
            if f.tell() == 0:
                writer.writerow(["start", "end", "EDA_Tonic_Mean", "EDA_Phasic_Mean","EDA_Mean"])  
             # Write the row with start_index, end_index, and the values from m
            writer.writerow([user_timestamps[idx][0], user_timestamps[idx][1], data['EDA_Tonic'].mean(), data['EDA_Phasic'].mean(), data['EDA_Raw'].mean()])
